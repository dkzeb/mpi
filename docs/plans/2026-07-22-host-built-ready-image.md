...
- `mode-selector.target` is the default and always opens a no-timeout menu,
  retrying until a cold MK3 attaches. D1/D2 or encoder/push selects a mode; D8
  persists the initially highlighted default.
...
- On first boot, before local filesystems mount, the physical-card remainder
  ...
  from a rootfs seed after those filesystems are recreated. Three ACT LED
  flashes repeat during the operation, while attached MK3 screens show milestone
  progress.
...
- [x] Build and inspect a fresh output image from a stock Raspberry Pi OS Lite
  ARM64 base.
- [ ] Hardware-validate cold-MK3 selector attach, both direct selections,
  persisted highlight, first-boot feedback, audio, display, HID, and repeated
  mode switching.
